...
export interface GooglePayConfig {
    ...
    allowedCardNetworks?: Array<'AMEX' | 'DISCOVER' | 'JCB' | 'MASTERCARD' | 'VISA' | 'INTERAC' | 'ELO'>;
...
export interface UseWalletPayReturn {
    ...
    processPayment: (config: WalletPayConfig, options?: ProcessPaymentOptions) => Promise<{
        success: boolean;
        result?: unknown;
        error?: Error;
    }>;
    processApplePayment: (config: ApplePayConfig) => Promise<{
        success: boolean;
        result?: unknown;
        error?: Error;
    }>;
    processGooglePayment: (config: GooglePayConfig) => Promise<{
        success: boolean;
        result?: unknown;
        error?: Error;
    }>;
...
export interface UseQuickPayReturn {
    quickPay: (config: Partial<WalletPayConfig> & Partial<ApplePayConfig> & Partial<GooglePayConfig>, processor?: PaymentProcessor) => Promise<PaymentProcessorResult | Record<string, unknown>>;
...
export interface WalletPayNativeModule {
    ...
    processGooglePayPayment?(config: Record<string, unknown>): Promise<PaymentResult>;
}
